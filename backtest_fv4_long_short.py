...
print("="*100)
print("FV4 Engine Backtest - Long/Short Mode (gross=1.0)")
print("="*100)
...
# 4. FV4 엔진 실행 (롱숏 모드, gross=1.0)
print("\n[4/5] Running FV4 engine (LONG/SHORT, gross=1.0)...")
cfg_fv4 = FV4Config(
    top_quantile=0.2,
    long_gross=0.5,      # 롱 50%
    short_gross=0.5,     # 숏 50%
    long_only=False      # 롱숏 모드
)
...
print(f"  Average gross: {np.mean(gross_list):.4f} (target: 1.0)")
print(f"  Average net: {np.mean(net_list):.4f} (target: 0.0)")
...
print("\n" + "="*100)
print("FV4 Engine Performance (LONG/SHORT, gross=1.0)")
print("="*100)
...
# 결과 저장
results = {
    'engine': 'FV4_LongShort',
    'description': 'Factor Value v4 with Signal Smoothing (Long/Short Mode, gross=1.0)',
    'config': {
...
with open('results/fv4_long_short_results.json', 'w') as f:
    json.dump(results, f, indent=2)

print("\n✅ Results saved to results/fv4_long_short_results.json")
print("\n" + "="*100)
print("FV4 Long/Short Backtest Complete!")
print("="*100)
