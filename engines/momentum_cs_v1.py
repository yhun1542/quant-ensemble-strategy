#!/usr/bin/env python3
"""
Cross-sectional Momentum Engine v1
...
@dataclass
class MomentumCSEngineV1Config:
...
class MomentumCSEngineV1:
    """
    Cross-sectional momentum engine (v1)
    - Universe: 30 mega-cap stocks (현재 유니버스)
    - Rebal: monthly (first trading day)
    - Long-only, top 6 names
    """

    def __init__(self, config: MomentumCSEngineV1Config = None):
        self.config = config or MomentumCSEngineV1Config()
    ...
    def compute_momentum_factors(self, prices: pd.DataFrame) -> pd.DataFrame:
        ...
        price_lag_long = prices.shift(cfg.lookback_long)
        price_lag_ex = prices.shift(cfg.lookback_exclude)
        mom_252_ex_21 = price_lag_ex / price_lag_long - 1.0

        # 단기 모멘텀
        price_lag_short = prices.shift(cfg.lookback_short)
        mom_21 = prices / price_lag_short - 1.0
...
def main():
    print("=" * 100)
    print("Momentum CS v1 Engine - 30 Mega-cap Universe")
    print("=" * 100)
    ...
    output_path = output_dir / "momentum_cs_v1_oos.json"
